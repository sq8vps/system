        ...
        switch(*((uint8_t*)entry))
        {
            case MP_CONFIGURATION_TABLE_ENTRY_CPU:
                ...
                if(0 == (cpu->cpuFlags & MP_CPU_ENTRY_FLAG_EN))
                    break;
                HalCpuConfigTable[HalCpuCount].apicId = cpu->localApicId;
                HalCpuConfigTable[HalCpuCount].boot = (cpu->cpuFlags & MP_CPU_ENTRY_FLAG_BP) > 0;
                HalCpuConfigTable[HalCpuCount].usable = (cpu->cpuFlags & MP_CPU_ENTRY_FLAG_EN) > 0;
                HalCpuCount++;
