...
static uint8_t RtlFindHighestPositionBase10(uint64_t n, uint64_t *power)
{
    *power = 1;
    uint8_t pos = 0;
    do
...
static uint8_t RtlFindHighestPositionBase2(uint64_t n, uint8_t baseExponent)
{
    if (0 == baseExponent)
        return 0;
    uint64_t pos = 0;
...
static int RtlPrintPad(struct RtlVPrintfConfig *config, uint32_t count, bool zeroPad, size_t alreadyWritten)
{
    ...
    {
        RtlMemset(&(config->buffer[alreadyWritten]), zeroPad ? '0' : ' ', count);
        return count;
    }
}
...
static int RtlPrintNumber(struct RtlVPrintfConfig *config, uint64_t x, bool sign, uint32_t width,
                        bool usePrecision, uint32_t precision, enum RtlPrintFlag flag,
                        uint8_t baseExponent, bool upperCase, size_t alreadyWritten)
{
    ...
    if (0 == baseExponent)
        length = RtlFindHighestPositionBase10(x, &power);
    else
    {
        length = RtlFindHighestPositionBase2(x, baseExponent);
...
static int RtlVprintf(struct RtlVPrintfConfig config, const char *format, va_list args)
{
    ...
    while (*format)
    {
        ...
        {
            uint8_t k = 1;
            enum RtlPrintFlag pFlag = NO_FLAG;
            // analyze flags
            switch (format[k])
            {
            case '-':
            ...
            if (('i' == format[k]) || ('d' == format[k]))
            {
                ...
                written += RtlPrintNumber(&config, RtlAbs(x), x >= 0, width, precisionSpecified, precision, pFlag, 0, false, written);
            }
            else if (('u' == format[k]) || ('o' == format[k]) || ('x' == format[k]) || ('X' == format[k]) || ('b' == format[k]) || ('B' == format[k]))
            {
                ...
                {
                case 'u':
                    written += RtlPrintNumber(&config, x, true, width, precisionSpecified, precision, pFlag, 0, false, written);
                    break;
                case 'o':
                    written += RtlPrintNumber(&config, x, true, width, precisionSpecified, precision, pFlag, 3, false, written);
                    break;
                case 'x':
                case 'X':
                    written += RtlPrintNumber(&config, x, true, width, precisionSpecified, precision, pFlag, 4, 'X' == format[k], written);
                    break;
                case 'b':
                case 'B':
                    written += RtlPrintNumber(&config, x, true, width, precisionSpecified, precision, pFlag, 1, 'B' == format[k], written);
                    break;
                }
            }
            else if ('c' == format[k])
            ...
            else if ('s' == format[k])
            {
                ...
                if (LEFT_JUSTIFY == pFlag)
                {
                    if (toWrite < width)
                    {
                        ...
                        written += RtlPrintPad(&config, padLength, false, written);
                    }
                }
            }
            else if ('p' == format[k])
