...
static void setupVgaRegisters(uint8_t *regs)
{
	...
	IoPortWriteByte(BOOTVGA_MISC_WRITE, *regs);
	regs++;

    /* unlock CRTC registers */
	for (i = 0; i < BOOTVGA_NUM_SEQ_REGS; i++)
	{
		IoPortWriteByte(BOOTVGA_SEQ_INDEX, i);
		IoPortWriteByte(BOOTVGA_SEQ_DATA, *regs);
		regs++;
	}
	/* unlock CRTC registers */
	IoPortWriteByte(BOOTVGA_CRTC_INDEX, 0x03);
	IoPortWriteByte(BOOTVGA_CRTC_DATA, IoPortReadByte(BOOTVGA_CRTC_DATA) | 0x80);
	IoPortWriteByte(BOOTVGA_CRTC_INDEX, 0x11);
	IoPortWriteByte(BOOTVGA_CRTC_DATA, IoPortReadByte(BOOTVGA_CRTC_DATA) & ~0x80);
	...
	{
		IoPortWriteByte(BOOTVGA_CRTC_INDEX, i);
		IoPortWriteByte(BOOTVGA_CRTC_DATA, *regs);
		regs++;
	}
	/* write GRAPHICS CONTROLLER regs */
	for (i = 0; i < BOOTVGA_NUM_GC_REGS; i++)
	{
		IoPortWriteByte(BOOTVGA_GC_INDEX, i);
		IoPortWriteByte(BOOTVGA_GC_DATA, *regs);
		regs++;
	}
	/* write ATTRIBUTE CONTROLLER regs */
	for (i = 0; i < BOOTVGA_NUM_AC_REGS; i++)
	{
		IoPortReadByte(BOOTVGA_INSTAT_READ);
		IoPortWriteByte(BOOTVGA_AC_INDEX, i);
		IoPortWriteByte(BOOTVGA_AC_WRITE, *regs);
		regs++;
	}

	/* lock 16-color palette and unblank display */
	IoPortReadByte(BOOTVGA_INSTAT_READ);
	IoPortWriteByte(BOOTVGA_AC_INDEX, 0x20);
}

static void writeColorPalette(void)
{
	IoPortWriteByte(BOOTVGA_AC_INDEX, 0x00); //enable access to color palette
    IoPortWriteByte(BOOTVGA_DAC_WRITE_INDEX, 0);
    
    uint8_t r = 0, g = 0, b = 0;
#ifdef BOOT_VGA_USE_640_480
    for(uint16_t i = 0; i < 16; i++)
    {
        IoPortWriteByte(BOOTVGA_DAC_DATA, r);
        IoPortWriteByte(BOOTVGA_DAC_DATA, g);
        IoPortWriteByte(BOOTVGA_DAC_DATA, b);
...
#endif
	IoPortWriteByte(BOOTVGA_AC_INDEX, 0x20); //disable access to color palette
}
...
void BootVgaDeinit(void)
{
	...
	IoPortWriteByte(BOOTVGA_GC_INDEX, 0x08);
    IoPortWriteByte(BOOTVGA_GC_DATA, 0xff);
}
...
static inline void setPlane(uint8_t plane)
{
	...
	IoPortWriteByte(BOOTVGA_GC_INDEX, 4);
	IoPortWriteByte(BOOTVGA_GC_DATA, plane);

	IoPortWriteByte(BOOTVGA_SEQ_INDEX, 2);
	IoPortWriteByte(BOOTVGA_SEQ_DATA, mask);
}
#endif
