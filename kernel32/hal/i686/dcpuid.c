...
static uint32_t CpuidReadEdx(uint32_t page)
{
    if(!cpuidAvailable)
        return 0;
    uintptr_t dummy;
...
static uint32_t CpuidReadEcx(uint32_t page)
{
    if(!cpuidAvailable)
        return 0;
    uintptr_t dummy;
...
{
    return (CpuidReadEdx(1) & CPUID_FEATURE_EDX_APIC) > 0;
}

bool CpuidCheckIfTscAvailable(void)
{
    return (CpuidReadEdx(1) & CPUID_FEATURE_EDX_TSC) > 0;
}

bool CpuidCheckIfTscDeadlineAvailable(void)
{
    return (CpuidReadEcx(1) & CPUID_FEATURE_ECX_TSC) > 0;
}
...
bool CpuidCheckIfTscInvariant(void)
{
    ...
    return (CpuidReadEdx(0x80000007) & CPUID_EXFEATURE_EDX_TSC_INVARIANT) > 0; 
}

bool CpuidCheckIfMsrAvailable(void)
{
    return (CpuidReadEdx(1) & CPUID_FEATURE_EDX_MSR) > 0;
}

bool CpuidCheckIfFpuAvailable(void)
{
    return (CpuidReadEdx(1) & CPUID_FEATURE_EDX_FPU) > 0; 
}

bool CpuidCheckIfMmxAvailable(void)
{
    return (CpuidReadEdx(1) & CPUID_FEATURE_EDX_MMX) > 0; 
}
